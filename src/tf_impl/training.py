...
def train_network(
    ...
    """
    Train the autoencoder network (including SINDy).
    ...
    Returns:
        results_dict: Dictionary containing the results of the training process.
    """
    # SET UP NETWORK
    ...
    train_op_model_refinement = tf.train.AdamOptimizer(
        learning_rate=learning_rate
    ).minimize(loss_refinement)
    ...
    loss_feature_names = [
        ...
        "SINDy_z loss",
        "SINDy_x loss",
        "Sindy Regularisation- L1 Norm",
        "Autoencoder weights- L1 Norm",
        "Classification Loss",
    ]
    ...
    with tf.Session() as sess:
        ...
        for i in range(params["max_epochs"]):
            ...
            for j in range(params["epoch_size"] // params["batch_size"]):
                ...
                batch_idxs = np.arange(
                    j * params["batch_size"], (j + 1) * params["batch_size"]
                )

                # Create feed dictionary for training data
                train_dict = create_feed_dictionary(
                    training_data, params, idxs=batch_idxs
                )
                sess.run(train_op, feed_dict=train_dict)

            # Calculate the validation predictions for z
            z_validation = sess.run(
                autoencoder_network["dz"], feed_dict=validation_dict
            )
            ...
            if params["print_progress"] and (i % params["print_frequency"] == 0):
                train_loss, val_loss, ratio = print_progress(
                    sess,
                    i,
                    loss,
                    losses,
            ...
            if (
                params["sequential_thresholding"]
                and (i % params["threshold_frequency"] == 0)
                and (i > 1)
            ):
                params["coefficient_mask"] = (
                    np.abs(
                        sess.run(autoencoder_network["sindy_coefficients"])
                    )
                ...
                print(
                    ...
                    % np.sum(params["coefficient_mask"])
                )
        ...
        for i_refinement in range(params["refinement_epochs"]):
            ...
            # Check if printing progress is required for refinement phase
            if params["print_progress"] and (
                i_refinement % params["print_frequency"] == 0
            ):
                train_loss, val_loss, ratio = print_progress(
                    sess,
                    i_refinement,
        ...
        pickle.dump(
            params,
            open(params["save_folder"] + params["save_name"] + "_params.pkl", "wb"),
        )
        ...
        sindy_coefficients = sess.run(
            autoencoder_network["sindy_coefficients"], feed_dict={}
        )
        ...
        plot_curves(
            start,
            title=params["save_name"] + "errors",
            feature_names=loss_feature_names,
            params=title_params,
            axis_name=y_axis_loss,
            training_array=training_array,
            validation_exists=True,
            validation_array=validation_array,
            scale=params["print_frequency"],
        )
        plot_curves(
            start,
            title=params["save_name"] + "errors_ref",
            feature_names=loss_feature_names,
            params=title_params_ref,
            axis_name=y_axis_loss,
        ...
        plot_curves(
            start,
            params["save_name"] + "ratios",
            feature_names=ratio_feature_names,
            params=title_params,
            axis_name=y_axis_ratios,
            training_array=ratios_array,
            validation_exists=False,
            scale=params["print_frequency"],
        )
        plot_curves(
            start,
            params["save_name"] + "ratios_ref",
            feature_names=ratio_feature_names,
            params=title_params_ref,
            axis_name=y_axis_ratios,
            training_array=ref_ratios_array,
            validation_exists=False,
            scale=params["print_frequency"],
        )
...
def create_feed_dictionary(
    data: Dict[str, np.ndarray], params: Dict[str, Any], idxs: Optional[np.ndarray] = None
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        idxs: Optional indices to select from dataset.

    Returns:
        feed_dict: Dictionary containing data to pass to tensorflow.
    """
    if idxs is None:
